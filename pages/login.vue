// /pages/login.vue - V12.5 - CORREÇÃO CRÍTICA: Substitui a chamada `$auth.login` pela Store Pinia para resolver o erro "Provider de autenticação ausente".
<template>
...
<script setup>
// /pages/login.vue - V12.5 - CORREÇÃO CRÍTICA: Substitui $auth pela chamada direta à Store Pinia para resolver o erro "Provider de autenticação ausente".
...
try {
// REMOVIDO: const { $auth } = useNuxtApp(); // Não é mais necessário, usando authStore diretamente
...
// Mapeamento de campos da API para o formato esperado pela Pinia Store
// Sua Store espera { token, user: AnyUser }
// Onde AnyUser tem id, userId, cpf, nome, sobrenome, email, telefone, roleLevel, roleName, cotistaId
const userPayloadForStore = {
id: apiResponse.userId, 
userId: apiResponse.userId, // Duplicado conforme sua Store normaliza (id ?? userId)
cpf: apiResponse.cpf, 
nome: apiResponse.nome, 
sobrenome: apiResponse.sobrenome,
email: apiResponse.email || '', 
// Sua API não retorna 'telefone'. Garantir que não cause erro na Store.
telefone: apiResponse.telefone || '', // Adicionado conforme AnyUser esperado na Store
roleLevel: apiResponse.roleLevel, 
roleName: apiResponse.roleName,
cotistaId: apiResponse.cotistaId || null 
};

const authPayloadForStore = {
token: apiResponse.token, 
user: userPayloadForStore
};

// ✅ CORREÇÃO V12.5: Chamada direta ao método 'login' da Store Pinia
if (typeof authStore.login === 'function') {
  authStore.login(authPayloadForStore); // Chama o método login da sua Store Pinia
...
<style scoped>
/* /pages/login.vue - V12.5 - Estilos específicos para o componente */
/* Os estilos do checkbox foram removidos do template na V12.4. */
