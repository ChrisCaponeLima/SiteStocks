// /server/api/admin/users/index.get.ts - V3.3 - FIX: Mapeamento de data para 'dataCriacao' para compatibilidade com Frontend.
...
export default defineEventHandler(async (event) => {
 try {
  ...
  // -------------------------------------------------------------------------
    // ✅ AJUSTE CRÍTICO: Mapeia ultimoAcesso para dataCriacao (o frontend espera 'dataCriacao').
  const finalUsers = users.map((u) => ({
   ...u,
      dataCriacao: u.ultimoAcesso, // ⬅️ Corrigido o mapeamento para não quebrar o frontend
