// /server/api/savings/boxes/process-earnings.post.ts - V1.3.1 ROBUSTO
// 🔒 Implementa checagem Decimal, idempotência via Movements e registro de duração.
// 🧩 Incluído log de inicialização para depuração e confirmação de carregamento do endpoint.
...
// 🟢 Log de carregamento do endpoint no momento em que o servidor é inicializado
console.log('[INIT][CRON] Endpoint /api/savings/boxes/process-earnings carregado com sucesso e aguardando execução.')
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    for (const box of activeBoxes) {
      ...
      // 🔑 Idempotência 2: Checa se uma movimentação de RENDIMENTO já foi registrada hoje
